...
contract IdeaMarketplace is ERC721 {
    ...
    function listNftForFixedPrice(uint256 _tokenId, uint256 _nftPrice, address _nftContractAddress, uint256 _royaltyFeePercentage) external OnlyTokenHolders(_tokenId, _nftContractAddress) returns (uint256) {
        ...
        require(block.timestamp <= expiryTime, "NFT have expired");
        ...
        if (fixedPrice[_tokenId].royaltyFeePercentage == 0) {
            if (msg.sender == originalCreator) {
                fixedPrice[_tokenId].royaltyFeePercentage = _royaltyFeePercentage;
            }   
        }

        uint256 effectiveRoyaltyFee = fixedPrice[_tokenId].royaltyFeePercentage;
    ...
    function buyFixedPriceNft(uint256 _fixedId) external payable {
        ...
        uint256 expiryTime = ideaNftContract.getNFTExpireTime(fixedPrice[_fixedId].tokenId);
        require(block.timestamp <= expiryTime, "NFT have expired");
        ...
        if (ideaNftContractAddress == fixedPrice[fixedPriceId].nftAddress) {
            ...
            address _royaltyReciever = ideaNftContract.getRoyaltyReciever(fixedPrice[_fixedId].tokenId);
    ...
    modifier OnlyTokenHolders(uint256 _tokenId, address _nftContractAddress) {
        ...
        require(IERC721(_nftContractAddress).ownerOf(_tokenId) != address(0), "Owner of this token Id does not exists");
        _;
    }
    ...
    function cancelListingForFixedPrice(uint256 _listingId) external {
        ...
        uint256 expiryTime = ideaNftContract.getNFTExpireTime(fixedPrice[_listingId].tokenId);
        require(block.timestamp <= expiryTime, "NFT have expired");
